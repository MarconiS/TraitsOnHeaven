...
stack_isotopes_leaf_products <- function(prd=NULL){
  product_name = list.files(paste("./TOS_Retriever/tmp/filesToStack", prd,"/stackedFiles", sep=""))
  product_name = product_name[-which(product_name %in% c("validation.csv", "variables.csv"))]
  final_isotopes_data = read_csv(paste("./TOS_Retriever/tmp/filesToStack", prd,"/stackedFiles/", product_name, sep="")) %>%
    dplyr::select(c(sampleID, d15N, d13C)) %>%
    write_csv('./TOS_Retriever/out/isotopes_data.csv')                          
}
