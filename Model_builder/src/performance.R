...
if(get100){
  ...
  for(bb in 1: length(mod.aic)){
    mod.aic[bb] <- foo[[bb]]$mod$FinalModel$aic
  ...
  plsglm <- foo[mask[,1]]
  save(plsglm, file = paste(out_dir, "plsglm_daic", j,  exp_itc, sep = ""))
}else{
  #load(file=paste("//orange/ewhite/s.marconi/traitsOnHeaven/original_pls_glm/plsglm_daic", j,  sep = ""))
  load(file=paste(out_dir, "/plsglm_daic", j, exp_itc,  sep = ""))
}
...
for(bb in 1: length(plsglm)){
  ...
  mod.r2[bb] <- 1 - sum((plsglm[[bb]]$pred - (plsglm[[bb]]$score$data$Y.test))^2) /
...
if(normz==T){
  foo <-t(diff(t(log(spectra_log_dif_snv[,-c(1:nsites)])),differences=1, lag=3))
  spectra_log_dif_snv <- cbind(spectra_log_dif_snv[,c(1:nsites)], foo)
}
