...
#pre-2015
extract_crown_data_old <- function(centroids, hps_f, f_path, chm_f, epsg, wd,NeonSites =NULL,  buffer = 20, cores = 2){
  library(foreach)
  library(doParallel)
  library(rhdf5)
  ...
  for(ii in 1:length(hps_f)){
    cr_per_path[ii] <- tryCatch(
      {dim(get_itcs_in_tile2015(hps_f[ii], centroids=centroids, NeonSites = NeonSites, f_path=f_path))[1]},
      error=function(cond){
        message(paste("hiperspectral flight path corrupted or incoplete:", ii))
        return(0)
      })
  ...
  for(z in 1:length(clean_hps)){ 
    ...
    for(mm in 1:cr_per_path[z]){
      ...
      # print(search())
      #"/Users/sergiomarconi/anaconda3/bin/"
      token = unlist(strsplit(clean_hps[z], split = "_"))[3] #3 for old tiles 6 for new
      itcextract <- get_itcs_in_tile2015(clean_hps[z], centroids, NeonSites= NeonSites, f_path)
      tryCatch({chm_pt <- list.files(chm_f, pattern = paste(as.integer(itcextract$easting[mm]/1000)*1000, "_",
                                                            as.integer(itcextract$northing[mm]/1000)*1000, sep=""))
      chm <- (paste(chm_f, chm_pt, sep="/"))
      itcExtract2015(x = itcextract[mm,], f= paste(f_path,clean_hps[z],sep = "/"), chm = chm, buffer = buffer,
                     epsg = epsg, token = token, wd = wd, pybin = "/home/s.marconi/.conda/envs/quetzal3/bin")
      print(paste(mm, z, clean_hps[z]))
      },error=function(e){})
    }
