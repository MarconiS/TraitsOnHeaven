...
#32611
itcExtract2015 <- function(x, f, chm, itc.f, epsg, token, wd,pybin = "/home/s.marconi/.conda/envs/quetzal3/bin",  buffer = 20){
  library(raster)
  library(rgeos)
  library(rgdal)
  library(readr)
  ...
  Sys.setenv(PATH = paste(pybin, Sys.getenv("PATH"),sep=":"))
  system2("python3", args=(sprintf('"%1$s" "%2$s" "%3$s" "%4$s" "%5$f" "%6$f" "%7$f" "%8$f" "%9$s" "%10$s"',paste(wd, "src/extractCrown_old.py", sep=""), 
                                   f, chm, entryID, clip.xmin, clip.xmax, clip.ymin, clip.ymax, as.character(epsg), wd)))
  clip<- stack(paste(wd, "/outputs/itcTiff/", entryID, '.tif', sep=""))
}
