...
get_buffer_itc <- function(){
  ...
  file_path <- "/Users/sergiomarconi/Documents/Chapter2/JointTraitsDist/Extracted_by_NEON/outputs/itcTiff/"
  wd = "/Users/sergiomarconi/Documents/Chapter2/JointTraitsDist/Extracted_by_NEON/"
  flist <- list.files(file_path)
  #flist <- flist[-grep("OSBS", flist)]
  
  # estimate crown allometry from dbh, where needed
  tos_data <- crowne_size_data <- readr::read_csv("~/Documents/GitHub/TraitsOnHeaven/TOS_Retriever/out/field_data.csv") %>%
